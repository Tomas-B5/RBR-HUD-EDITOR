...
def reload_controls(window, values):
    #Update sliders:
    window["-SLIDER-BACKGROUND-"].update(window["-SLIDER-BACKGROUND-"].metadata.Alpha) 
...
def Init_GUI(dash):
    ...
    # Create the Window
    window = sg.Window('RBR HUD editor. For RallySimFans launcher', layout, finalize=True, return_keyboard_events=True, use_default_focus=True)
...
def Build_layout(dash):
    controls_col = [[sg.Text('RBR Folder'), sg.In(size=(25, 1), enable_events=True, key='-FOLDER-', disabled=True), sg.FolderBrowse()],
                    [sg.Text('Background :'),
                    ...
                    [sg.Text('Position:'), sg.In(key='-X-', size=(8, 1)), sg.In(key='-Y-', size=(8, 1)), sg.Button('Set')],
                    [sg.Text('Step:'), sg.Button('0.1'), sg.Button('1'), sg.Button('10'), sg.Button('100')],
                    [sg.Button('UP')],
                    [sg.Button('LEFT'), sg.Button('RIGHT')],
                    [sg.Button('DOWN')],
    ]
    left_col = [
        [sg.Listbox(values=[], enable_events=True, size=(40, 20), key='-Value List-')]]
    layout = [[sg.Column(left_col, element_justification='left'), sg.VSeperator(
